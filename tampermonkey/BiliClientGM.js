export class BiliClientGM {
    ...
    async init() {
        ...
        const wbi = navData.data?.wbi_img;
        this.imgKey = wbi?.img_url?.split('/').pop()?.split('.')[0] || '';
        this.subKey = wbi?.sub_url?.split('/').pop()?.split('.')[0] || '';
    }
    async ensureBuvid3() {
            ...
            GM_xmlhttpRequest({
                ...
                onload: res => {
                    try {
                        ...
                        this.buvid3 = json?.data?.buvid;
                        if (this.buvid3) {
                            console.log("✅ 获取 buvid3:", this.buvid3);
                            resolve(this.buvid3);
                        } else {
                            reject("未获取到 buvid3");
                        }
    ...
    signParams(params = {}) {
        const mixinKey = this.getMixinKey(this.imgKey + this.subKey);
        params.wts = Math.floor(Date.now() / 1000); // 时间戳
        // 过滤特殊字符
        const filtered = {};
    ...
    async request({ url, params = {}, sign = false, desc = '', responseType = 'json' }) {
        ...
        const doRequest = (useCookie = false) => new Promise((resolve, reject) => {
            const headers = { ...this.headers };
            if (useCookie && this.buvid3) {
